function test_HOG(SVM, test_pos_path, scale, win_size, step_size, label_path)
  pos = 1;
  padding = [0 0];
  overlap_th = 0.3;
  ...
  pos_hog = zeros(n_wins, 3780); % 105*4*9
  parfor i=1:n_wins
    pos_hog(i,:) = extractHOGFeatures(pos_wins{i});
  end

    [test_label, score] = predict(SVM, pos_hog);
    ...
    save(sprintf('hog_pick_%d', n_last_img), 'pick');
